...
#define PREFIX_PAGE_AZLYRICS "http://search.azlyrics.com/search.php?q"
#define PREFIX_LRC_AZLYRICS "http://search.azlyrics.com"
#define LRC_CLUE_AZLYRICS "downlrc"
...
ol_lrc_fetch_azlyrics_search(const OlMusicInfo *info, int *size, const char* charset)
{
  if(info == NULL)
    return NULL;
  ol_debugf ("  title: %s\n"
  ...
  strcpy(page_url, PREFIX_PAGE_AZLYRICS);
  if(info->title != NULL) {
  ...
  while(fgets(buf, BUFSIZE, fp) != NULL && count<TRY_MATCH_MAX) {
    if((ptr = strstr(buf, LRC_CLUE_AZLYRICS)) != NULL) {
      tp = strchr(ptr, '"');
      if(tp != NULL)
        *tp = 0;
      tp = strrchr(ptr, '=');
      ...
      *(--p) = 0;
      ...
      /* restore the url */
      *p = '-';
      strcpy(candidate.url, PREFIX_LRC_AZLYRICS);
...
int 
ol_lrc_fetch_azlyrics_download(OlLrcCandidate *tsu, const char *pathname, const char *charset)
{
  ol_log_func ();
...
static OlLrcFetchEngine azlyrics = {
  N_("AZLyrics"),
  ol_lrc_fetch_azlyrics_search,
  ol_lrc_fetch_azlyrics_download,
};

OlLrcFetchEngine *ol_lrc_fetch_azlyrics_engine ()
{
  return &azlyrics;
}
